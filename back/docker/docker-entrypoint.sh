...
if [ ! -s ${JDX_DIR}/config/JDInfoConfig.json ]; then
  echo "检测到config配置目录下不存在JDInfoConfig.json，从示例文件复制一份用于初始化...\n"
  cp -fv ${JDX_DIR}/sample/JDInfoConfig.json ${JDX_DIR}/config/JDInfoConfig.json
fi
