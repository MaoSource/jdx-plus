...
public class JdService {
    ...
    public String addXdd(String cookie, long qq) {
        cookie = cookie.replaceAll(";","%3B");
        String param = StrUtil.format("token={}&ck={}&qq={}", systemConfigProperties.getXddToken(), cookie, qq);
        HttpResponse response = HttpRequest.post(systemConfigProperties.getXddUrl())
                .body(param)
                .execute();
        JSONObject responseJo = JSONObject.parseObject(response.body());
        log.info(responseJo.getString("message"));
        return responseJo.getString("message");
    }

    public static void main(String[] args) {
        String aa = "pt_key=AAJiHuAsADDblqiBiiJ;pt_pin=jd_ghSMQ;";
//        log.info((String) JSONObject.toJSON(aa));
        System.out.println(JSONObject.toJSON(aa));
//        String aaa = addXdd(aa, 1134496928);
    }
}
